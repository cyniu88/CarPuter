...
// Debugging...  
  ...
  void IA() {
    if ((flagPID == false) && (velocity < (velocityFaixa + FAIXA)) && (velocity > (velocityFaixa - FAIXA)))
      countInterations++;
    else {
      countInterations = 0;
    ...
    if ((flagPID == false) && (countInterations > INTERATIONS)) {
        digitalWrite(LED, HIGH);
        if (count > INTERATIONS) {
            countInterations = 0;
            digitalWrite(LED, LOW);
            count = 0;
        }
